...
POST: Abrir um novo terminal e executar o comando:
curl.exe -X POST http://localhost:3000/users `
  -H "Content-Type: application/json" `
  -d '{"name":"Usuário4","email":"usuario4@email.com"}'

PUT: Abrir um novo terminal e executar o comando:
curl.exe -X PUT http://localhost:3000/users/2 `
  -H "Content-Type: application/json" `
  -d '{"name":"Nome Atualizado"}'

DELETE: curl.exe -X DELETE http://localhost:3000/users/id
